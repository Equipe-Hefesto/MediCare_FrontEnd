...
export default function RemedioCadastro4_3() {
  ...
  const [selected, setSelected] = useState(state.diasSemana);
  ...
  useEffect(() => {
    if (state.diasSemana?.length) {
      setSelected(state.diasSemana);
    }
  }, []);
  ...
  function handleChangeDiasSemana(index: number) {
    setSelected((prev) => {
      const updated =
        prev.includes(index) ? prev.filter((i) => i !== index) : [...prev, index];

      dispatch({ campo: "diasSemana", valor: updated });
      return updated;
    });
  }
  ...
  async function proximo() {
    ...
    if (valido) {
      const diasSemanaString = selected
        .sort((a, b) => a - b)
        .map((i) => ["Domingo", "Segunda", "Terça", "Quarta", "Quinta", "Sexta", "Sábado"][i]);
      ...
      const requestBody = {
        ...
        diasSemana: diasSemanaString, // <- Agora está certo
      ...
      >
        ...
        <View style={styles.scrollContainer}>
          ...
          <WeekDaySelector
            ...
            onSelect={handleChangeDiasSemana}
          />
